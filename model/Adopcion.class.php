...
class Adopcion extends Crud
{
    ...
    public function __construct ($idAnimal, $idUsuario, $fecha, $razon) {
    ...
    public function actualizar ()
    {
        ...
        foreach ($args as $key=> $value){
            if (property_exists(__CLASS__, $key)) {
                if ($key == "fecha" ) $this->$key = date('Y-m-d', strtotime($value));
                $this->$key = $value;
            }
        }
        ...
        try {
            ...
            $sql = "UPDATE " . self::TABLA . " SET fecha = :fecha, razon = :razon  WHERE id = :id LIMIT 1";
            //Creamos la consulta preparada desde el objeto de conexión de base de datos y le pasamos el SQL
            $stmt = $this->conexion->prepare($sql);
            //Bind value para calculos y expresiones
            //$stmt->bindValue(':make', '%' . $_GET['make'] . '%');
            ...
            if ($affected) {
                //Devuelve el último Id que se ha insertado
                echo $affected . "Adopción {$this->nombre} mofidicado con éxito";
            } else {
